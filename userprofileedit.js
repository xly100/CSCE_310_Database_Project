...
function queryAdmin(){ //Function queries whole row from user table for target userid
	let UID = getUserId(); 
	runPHP("getuser.php", {"userid":UID}, retrieveAdminInfo, alert);
}
...
function saveProfileDetails(){ //Called by "save button", stores text field values as variables and then sends to database
    ...
    runPHP("updateuserprofile.php", {"UID":UID, "FName":FName, "LName":LName, "PhoneNum":PhoneNum, "Username":Username, "Pwd":Pwd}, Usertype==="p"?patientUpdateDone:otherUpdateDone, alert);
